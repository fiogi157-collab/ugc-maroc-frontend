-- =====================================================
-- UGC MAROC - SIMPLE MIGRATION V2 (GESTION DES POLICIES EXISTANTES)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'payout_requests' AND policyname = 'Admins can view all payout requests') THEN
        DROP POLICY "Admins can view all payout requests" ON payout_requests;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'webhook_events' AND policyname = 'Only admins can view webhook events') THEN
        DROP POLICY "Only admins can view webhook events" ON webhook_events;
    END IF;
...
-- ===== SUCCESS MESSAGE =====
SELECT '✅ UGC Maroc migration V2 completed successfully! 9 new tables created with proper RLS policies.' as status;
