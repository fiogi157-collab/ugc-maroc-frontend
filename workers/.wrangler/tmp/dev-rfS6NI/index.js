...
var wallets = sqliteTable("wallets", {
  ...
  pending_balance: real("pending_balance").default(0).notNull(),
...
var campaignAgreements = sqliteTable("campaign_agreements", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  template_clauses: text("template_clauses"),
  // JSON string
  application_message: text("application_message"),
  portfolio_links: text("portfolio_links"),
  // JSON string
  delivery_days: integer("delivery_days"),
  additional_notes: text("additional_notes"),
  portfolio_files: text("portfolio_files"),
  // JSON string
...
var submissions = sqliteTable("submissions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  campaign_id: integer("campaign_id").notNull().references(() => campaigns.id, { onDelete: "cascade" }),
  creator_id: text("creator_id").notNull().references(() => profiles.id, { onDelete: "cascade" }),
...
var gigs = sqliteTable("gigs", {
  ...
  tags: text("tags"),
  // JSON string
  portfolio_urls: text("portfolio_urls"),
  // JSON string
  is_active: integer("is_active", { mode: "boolean" }).default(true),
  created_at: integer("created_at", { mode: "timestamp" }).default(sql`(unixepoch())`).notNull(),
  updated_at: integer("updated_at", { mode: "timestamp" }).default(sql`(unixepoch())`).notNull()
});
var negotiations = sqliteTable("negotiations", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var StreamService = class {
  ...
  async getVideoAnalytics(uid, options = {}) {
    const params = new URLSearchParams();
  ...
  async generateThumbnail(uid, options = {}) {
    const response = await fetch(
...
function createStreamRoutes(app2) {
  app2.use("/api/stream/*", async (c, next) => {
  ...
  app2.post("/api/stream/upload-url", async (c) => {
    const streamService = c.get("streamService");
    try {
      ...
      console.error("Stream upload URL error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/stream/video/:uid", async (c) => {
    const streamService = c.get("streamService");
    ...
    try {
      ...
      console.error("Stream get video error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/stream/videos", async (c) => {
    const streamService = c.get("streamService");
    try {
      ...
      console.error("Stream list videos error:", error3);
      return c.json({
        success: false,
  ...
  app2.delete("/api/stream/video/:uid", async (c) => {
    const streamService = c.get("streamService");
    const uid = c.req.param("uid");
    try {
      ...
      console.error("Stream delete video error:", error3);
      return c.json({
        success: false,
        error: "Failed to delete video",
        details: error3.message
  ...
  app2.post("/api/stream/watermark", async (c) => {
    const streamService = c.get("streamService");
    try {
      const formData = await c.req.formData();
      const file = formData.get("file");
      const name = formData.get("name");
      const opacity = parseFloat(formData.get("opacity"));
      ...
      console.error("Stream create watermark error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/stream/video/:uid/analytics", async (c) => {
    const streamService = c.get("streamService");
    const uid = c.req.param("uid");
    try {
      ...
      console.error("Stream analytics error:", error3);
      return c.json({
        success: false,
        error: "Failed to get analytics",
        details: error3.message
  ...
  app2.post("/api/stream/video/:uid/thumbnail", async (c) => {
    const streamService = c.get("streamService");
    const uid = c.req.param("uid");
    try {
      const body = await c.req.json();
      const { time: time3 = 0, size = { width: 640, height: 360 } } = body;
      ...
      console.error("Stream thumbnail error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/stream/webhook", async (c) => {
    try {
      const body = await c.req.json();
      switch (body.type) {
        case "video.ready":
      ...
      console.error("Stream webhook error:", error3);
      return c.json({
        success: false,
...
__name(createStreamRoutes, "createStreamRoutes");

// src/services/images.ts
var ImagesService = class {
  static {
    __name(this, "ImagesService");
  }
  accountId;
  apiToken;
  constructor(accountId, apiToken) {
  ...
  generateImageURL(imageId, variant) {
  ...
  generateImageURLWithParams(imageId, params = {}) {
    const baseUrl = `https://imagedelivery.net/${this.accountId}/${imageId}`;
...
function createImagesRoutes(app2) {
  app2.use("/api/images/*", async (c, next) => {
    const accountId = c.env.CLOUDFLARE_ACCOUNT_ID;
    const apiToken = c.env.CLOUDFLARE_API_TOKEN;
    if (!accountId || !apiToken) {
      return c.json({
  ...
  app2.post("/api/images/upload", async (c) => {
    const imagesService = c.get("imagesService");
    try {
      const formData = await c.req.formData();
      const file = formData.get("file");
      ...
      console.error("Images upload error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/images/:id", async (c) => {
    const imagesService = c.get("imagesService");
    ...
    try {
      ...
      console.error("Images get error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/images", async (c) => {
    ...
    try {
      ...
      console.error("Images list error:", error3);
      return c.json({
        success: false,
  ...
  app2.delete("/api/images/:id", async (c) => {
    const imagesService = c.get("imagesService");
    const id = c.req.param("id");
    try {
      ...
      console.error("Images delete error:", error3);
      return c.json({
        success: false,
        error: "Failed to delete image",
        details: error3.message
  ...
  app2.post("/api/images/variants", async (c) => {
    const imagesService = c.get("imagesService");
    try {
      const body = await c.req.json();
      const {
      ...
      console.error("Images create variant error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/images/variants", async (c) => {
    const imagesService = c.get("imagesService");
    try {
      ...
      console.error("Images list variants error:", error3);
      return c.json({
        success: false,
  ...
  app2.delete("/api/images/variants/:id", async (c) => {
    const imagesService = c.get("imagesService");
    const id = c.req.param("id");
    try {
      ...
      console.error("Images delete variant error:", error3);
      return c.json({
        success: false,
        error: "Failed to delete variant",
        details: error3.message
  ...
  app2.get("/api/images/stats", async (c) => {
    const imagesService = c.get("imagesService");
    try {
      ...
      console.error("Images stats error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/images/:id/url", async (c) => {
    const imagesService = c.get("imagesService");
    const id = c.req.param("id");
    try {
      const variant = c.req.query("variant");
      const width = c.req.query("width") ? parseInt(c.req.query("width")) : void 0;
      const height = c.req.query("height") ? parseInt(c.req.query("height")) : void 0;
      const fit = c.req.query("fit") || "cover";
      const quality = c.req.query("quality") ? parseInt(c.req.query("quality")) : void 0;
      const format = c.req.query("format") || "webp";
      const gravity = c.req.query("gravity") || "auto";
      let url;
      if (variant) {
      ...
      console.error("Images URL generation error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/images/setup-variants", async (c) => {
    const imagesService = c.get("imagesService");
    try {
      const variants = [
      ...
      for (const variant of variants) {
        try {
          ...
          console.warn(`Failed to create variant ${variant.id}:`, error3.message);
        }
      }
      return c.json({
        success: true,
      ...
      console.error("Images setup variants error:", error3);
      return c.json({
        success: false,
...
__name(createImagesRoutes, "createImagesRoutes");

// src/services/turnstile.ts
var TurnstileService = class _TurnstileService {
  static {
    __name(this, "TurnstileService");
  }
  secretKey;
  siteKey;
  ...
  async verifyWithOptions(token, options = {}) {
    try {
  ...
  async verifyWithRateLimit(token, options = {}, cache) {
    ...
    if (cache) {
      ...
      await cache.put(rateLimitKey, (attemptCount + 1).toString(), {
        expirationTtl: Math.floor(windowMs / 1e3)
      });
    }
    ...
    return {
      ...
      error: result.error,
      attempts: cache ? await cache.get(rateLimitKey).then((v) => v ? parseInt(v) : 0) : void 0
    };
  }
  ...
  async verifyWithAction(token, action, options = {}) {
    const result = await this.verifyWithOptions(token, {
      ...options,
      action: _TurnstileService.ACTIONS[action]
    });
    return {
      success: result.success,
      error: result.error,
...
function createTurnstileRoutes(app2) {
  app2.use("/api/turnstile/*", async (c, next) => {
  ...
  app2.post("/api/turnstile/verify", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const {
        token,
      ...
      console.error("Turnstile verify error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/verify-with-rate-limit", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const {
      ...
      console.error("Turnstile verify with rate limit error:", error3);
      return c.json({
        success: false,
        error: "Failed to verify token",
        details: error3.message
  ...
  app2.post("/api/turnstile/verify-action", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const {
      ...
      const result = await turnstileService.verifyWithAction(token, action, {
        remoteip,
        cdata
      });
      return c.json({
      ...
      console.error("Turnstile verify action error:", error3);
      return c.json({
        success: false,
        error: "Failed to verify token",
        details: error3.message
  ...
  app2.get("/api/turnstile/config", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      ...
      const config2 = turnstileService.getFrontendConfig({
        theme,
        size,
        language,
        callback,
      ...
      console.error("Turnstile config error:", error3);
      return c.json({
        success: false,
  ...
  app2.use("/api/protected/*", async (c, next) => {
  ...
  app2.post("/api/turnstile/signup", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      ...
      console.error("Turnstile signup error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/login", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const { token, remoteip } = body;
      ...
      console.error("Turnstile login error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/campaign-submit", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const { token, remoteip } = body;
      ...
      console.error("Turnstile campaign submit error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/gig-create", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const { token, remoteip } = body;
      ...
      console.error("Turnstile gig create error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/contact", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const { token, remoteip } = body;
      ...
      console.error("Turnstile contact error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/turnstile/password-reset", async (c) => {
    const turnstileService = c.get("turnstileService");
    try {
      const body = await c.req.json();
      const { token, remoteip } = body;
      ...
      console.error("Turnstile password reset error:", error3);
      return c.json({
        success: false,
...
__name(createTurnstileRoutes, "createTurnstileRoutes");

// src/services/email-routing.ts
var EmailRoutingService = class _EmailRoutingService {
  static {
    __name(this, "EmailRoutingService");
  }
  accountId;
  apiToken;
  constructor(accountId, apiToken) {
  ...
  async createRoute(pattern, destinations) {
    const response = await fetch(
  ...
  async updateRoute(id, pattern, destinations) {
    const response = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${this.accountId}/email/routing/rules/${id}`,
      {
        method: "PUT",
  ...
  async sendEmail(message, workerUrl) {
  ...
  static ROUTES = {
    SUPPORT: {
      ...
      destinations: [
        { type: "worker", value: "ugc-maroc-api" }
      ]
    },
    SALES: {
  ...
  async setupUgcMarocRoutes() {
    const routes = [];
    const errors = [];
    for (const [name, config2] of Object.entries(_EmailRoutingService.ROUTES)) {
      try {
        const route = await this.createRoute(config2.pattern, config2.destinations);
        routes.push(route);
        console.log(`\u2705 Created route: ${name} -> ${config2.pattern}`);
      } catch (error3) {
        const errorMsg = `Failed to create route ${name}: ${error3.message}`;
  ...
  async processIncomingEmail(emailData) {
    const { from, to, subject, text: text2, html, headers } = emailData;
    if (to.includes("support@")) {
      return this.handleSupportEmail({ from, subject, text: text2, html });
    } else if (to.includes("sales@")) {
      return this.handleSalesEmail({ from, subject, text: text2, html });
    } else if (to.includes("contact@")) {
      return this.handleContactEmail({ from, subject, text: text2, html });
    } else {
      return this.handleGeneralEmail({ from, to, subject, text: text2, html });
    }
  }
  async handleSupportEmail(data) {
    console.log("\u{1F4E7} Support email received:", data.from, data.subject);
    return {
      success: true,
      action: "support_ticket_created",
      response: "Votre demande de support a \xE9t\xE9 re\xE7ue. Nous vous r\xE9pondrons dans les 24h."
    };
  }
  async handleSalesEmail(data) {
    console.log("\u{1F4B0} Sales email received:", data.from, data.subject);
    return {
      success: true,
      action: "sales_inquiry_received",
      response: "Merci pour votre int\xE9r\xEAt ! Notre \xE9quipe commerciale vous contactera bient\xF4t."
    };
  }
  async handleContactEmail(data) {
    console.log("\u{1F4EC} Contact email received:", data.from, data.subject);
    return {
      success: true,
      action: "contact_form_received",
      response: "Merci pour votre message ! Nous vous r\xE9pondrons rapidement."
    };
  }
  async handleGeneralEmail(data) {
    console.log("\u{1F4E7} General email received:", data.from, data.to, data.subject);
    return {
      success: true,
      action: "email_forwarded",
      response: "Email re\xE7u et trait\xE9."
    };
  }
};

// src/routes/email-routing.ts
function createEmailRoutingRoutes(app2) {
  app2.use("/api/email/*", async (c, next) => {
    const accountId = c.env.CLOUDFLARE_ACCOUNT_ID;
    const apiToken = c.env.CLOUDFLARE_API_TOKEN;
    if (!accountId || !apiToken) {
      return c.json({
  ...
  app2.post("/api/email/routes", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      ...
      console.error("Email routing create error:", error3);
      return c.json({
        success: false,
  ...
  app2.get("/api/email/routes", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      ...
      console.error("Email routing list error:", error3);
      return c.json({
        success: false,
  ...
  app2.delete("/api/email/routes/:id", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    const id = c.req.param("id");
    try {
      ...
      console.error("Email routing delete error:", error3);
      return c.json({
        success: false,
        error: "Failed to delete email route",
        details: error3.message
  ...
  app2.put("/api/email/routes/:id", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    const id = c.req.param("id");
    try {
      const body = await c.req.json();
      ...
      console.error("Email routing update error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/setup-ugc-maroc", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      ...
      console.error("Email routing setup error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/send", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      const {
  ...
  app2.post("/api/email/process", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      if (body.type !== "email") {
      ...
      console.error("Email process error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/support", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      const { from, subject, text: text2, html } = body;
      ...
      console.error("Email support error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/sales", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      const { from, subject, text: text2, html } = body;
      ...
      console.error("Email sales error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/contact", async (c) => {
    const emailRoutingService = c.get("emailRoutingService");
    try {
      const body = await c.req.json();
      const { from, subject, text: text2, html } = body;
      ...
      console.error("Email contact error:", error3);
      return c.json({
        success: false,
  ...
  app2.post("/api/email/webhook", async (c) => {
    try {
      const body = await c.req.json();
      switch (body.type) {
        case "email.received":
          console.log("\u{1F4E7} Email received:", body.data.from, body.data.to);
          break;
        case "email.bounced":
          console.log("\u{1F4E7} Email bounced:", body.data.to, body.data.reason);
          break;
        case "email.complained":
          console.log("\u{1F4E7} Email complained:", body.data.to);
          break;
      }
      return c.json({ success: true });
    } catch (error3) {
...
__name(createEmailRoutingRoutes, "createEmailRoutingRoutes");
...
app.get("/api/profiles/:id", async (c) => {
  ...
  try {
    const profile3 = await db.select().from(schema.profiles).where(eq(schema.profiles.id, profileId)).get();
    if (!profile3) {
      return c.json({ error: "Profile not found" }, 404);
    }
    return c.json({ profile: profile3 });
  } catch (error3) {
...
app.get("/api/submissions", async (c) => {
  const db = getDb(c);
  try {
    const submissions2 = await db.select().from(schema.submissions).all();
    return c.json({ submissions: submissions2 });
  } catch (error3) {
...
app.get("/api/orders", async (c) => {
  const db = getDb(c);
  try {
    const orders2 = await db.select().from(schema.orders).all();
    return c.json({ orders: orders2 });
  } catch (error3) {
...
app.get("/api/payments", async (c) => {
  const db = getDb(c);
  try {
    const payments2 = await db.select().from(schema.payments).all();
    return c.json({ payments: payments2 });
  } catch (error3) {
...
app.get("/api/messages", async (c) => {
  const db = getDb(c);
  try {
    const messages2 = await db.select().from(schema.messages).all();
...
app.get("/api/negotiations", async (c) => {
  const db = getDb(c);
  try {
    const negotiations2 = await db.select().from(schema.negotiations).all();
    return c.json({ negotiations: negotiations2 });
  } catch (error3) {
...
app.get("/api/contracts", async (c) => {
  const db = getDb(c);
  try {
    const contracts2 = await db.select().from(schema.contracts).all();
    return c.json({ contracts: contracts2 });
  } catch (error3) {
...
// .wrangler/tmp/bundle-ozc5rl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ozc5rl/middleware-loader.entry.ts
